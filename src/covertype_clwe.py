...
def test_goldwasser(X_train, X_test, y_train, y_test, n_features, m):
    ...
    bk_goldwasser_c2, g_goldwasser = GP(n_features, m, c=2)
    ...
    print(f"Prediction flip ratio (with bk [c=2]): {flip_ratio:.4f}")



    bk_goldwasser_c3, g_goldwasser = GP(n_features, m, c=3)
    b_goldwasser = torch.rand(m, device=device)
...
def main():
    ...
    test_goldwasser(X_train, X_test, y_train, y_test, n_features=10, m=500)
